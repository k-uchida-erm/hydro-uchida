    ...
    try:
        ...
        for epoch in tqdm(range(epochs), desc="Training"):
            ...
            if (epoch + 1) % 10 == 0:  # 10エポックごとに記録
                ...
                # 進捗の表示（より詳細に）
                print(f"\rEpoch {epoch + 1}/{epochs} ({((epoch + 1)/epochs*100):.1f}%) | "
                      f"Total: {loss.item():.4f} | "
                      f"Train: {loss_train.item():.4f} | "
                      f"Val: {loss_obs.item():.4f} | "
                      f"LR: {new_lr:.2e}", end="")
...
def main():
    ...
    soil_map, df_bc, X_ic, h0, df_train, df_obs = load_all_data()
    
    # モデルの学習
    model = train(model, soil_map, df_bc, X_ic, h0, df_train, df_obs)
