...
- `analysis/visualize.py` を実行すると、バリデーション用のh-z分布比較グラフ（validation_profile.png）が自動生成されます。
    - 各時刻ごとに観測値（点）とモデル予測値（実線）が重ねて描画されます。
    - 保存先: `result/model_xxxx/plots/validation_profile.png`
- これにより、観測値とモデル予測値の鉛直分布を時刻ごとに比較できます。
