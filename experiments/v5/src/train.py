...
import pytz
from .config import *
from .loss import residual, bc_loss, ic_loss, obs_loss, theta_obs_loss, smoothness_near_surface, wetting_front_guided_loss
...
def train(model, soil_map, df_bc, X_ic, psi0, df_train, df_obs, X_res, epochs=EPOCHS):
    ...
    for rar_iter in range(5):
        ...
        for epoch in tqdm(range(epochs_per_rar), desc=f"RAR {rar_iter + 1}"):
            ...
            loss_surf = 0.002 * smoothness_near_surface(model, X_int)
            loss_front = 5.0 * wetting_front_guided_loss(model, df_train)
        ...
        if rar_iter < 4:
            ...
            if rar_iter == 0:
                ...
                t_early = np.linspace(0, 2, 25)   # 以前: 500
                z_early = np.linspace(0, 9, 11)   # 以前: 20
                for t in t_early:
                    for z in z_early:
                        new_points.append([0.0, 0.0, z, t])  # 約275点

                print("湿潤フロント近傍（z=7-9cm）の重点サンプリング中...")
                t_wetfront = np.linspace(0, 2, 60)  # 以前: 1000
                z_wetfront = np.linspace(7, 9, 40)  # 以前: 100
                for t in t_wetfront:
                    for z in z_wetfront:
                        new_points.append([0.0, 0.0, z, t])  # 約2400点

                print("深い部分（z=8-10cm）の重点サンプリング中...")
                t_deep = np.linspace(0, 10, 100)  # 以前: 2000
                z_deep = np.linspace(8, 10, 40)   # 以前: 200
                for t in t_deep:
                    for z in z_deep:
    ...
    meta = {
        ...
        'rar': {
            'N_res': RAR_N_RES,
            'focus_ratio': RAR_FOCUS_RATIO
        },
        'bc': {
            'neumann_flux': BC_NEUMANN_FLUX
        }
    }
    try:
...
def train_bfgs(model, soil_map, df_bc, X_ic, psi0, df_train, df_obs, X_res, model_dir):
    ...
    def closure():
        ...
        loss_surf = 0.002 * smoothness_near_surface(model, X_res)
        loss_front = 5.0 * wetting_front_guided_loss(model, df_train)
